...
- `f8bd046`: controles de gol, exclusión, tarjeta amarilla y tarjeta roja.
  ...
  - `HEAD`: el banner inferior usa AuthBloc como fuente de sesión en todas las vistas, normaliza roles públicos y administrativos, conserva índices válidos y usa rutas de `go_router` tanto para Home como para Calendario; se eliminó la mezcla con `Navigator.push` que dejaba Calendario encima de Home. Calendario está declarado como ruta pública para el rol `publico`.
  - `HEAD`: la navegación principal usa GoRouter para Home, Calendario, Perfil y Árbitros; el banner ya no crea pantallas con `Navigator.push`, evitando pilas inconsistentes y el error `There is no GoRouterState above the current context`. Calendario, detalle de torneo (`/torneos/detalle`) y Partido en vivo (`/partido-en-vivo`) son rutas públicas para consultar la información sin iniciar sesión. El rol `publico` también permanece en esas rutas y vuelve a Home desde Splash, sin ser enviado a Perfil o Login.
  - `HEAD`: los registros pendientes de jugador también se vinculan por correo al UID real cuando crean cuenta, evitando duplicados y conservando sus datos.
  - `HEAD`: el esquema canónico de usuarios usa `email`, `displayName`, `phone`, `document` y `accreditation`; los nuevos registros ya no duplican esos datos con `correo`, `nombre`, `telefono` o `nivel`.
  - `HEAD`: completar perfil verifica en el servidor que Firestore confirmó `profileCompleted` o `profileCompletedByRole`; normaliza todos los roles y las reglas de Firestore permiten actualizar el perfil propio aunque el documento anterior no tuviera `uid` o `roles` completos. El rol `publico` nunca se obliga a completar perfil.
