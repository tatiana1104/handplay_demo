...
- [x] Navegación protegida por sesión y roles (`redirect` de go_router según `AuthBloc`; roles disponibles desde custom claims de Firebase). La ruta de creación valida `state.extra` y usa el UID autenticado como respaldo; `main.dart` importa explícitamente `firebase_auth` para resolver ese UID. Las fechas de inicio, fin y límite de inscripción no pueden ser anteriores al día actual, tanto en el selector como al guardar.
